...
import { LOCALSTORAGE_KEYS, USER_ROLES, MENU_ITEM, SESSIONSTORAGE_KEYS } from '../../utils/constants';
import { generateReferalCode } from '../../utils/functions';
...
const SignUp = props => {
  ...
  useEffect(() => {
    signUp(router, routerLocationHash, auth);
...
const signUp = async (router, routerLocationHash, auth) => {
  if (routerLocationHash.access_token) {
    ...
    // User params form cognito needed to create a new user in DB
    const userParams = {
      cognitoUserId: tokenDecodedObj.sub,
      name: getName(tokenDecodedObj),
      email: tokenDecodedObj.email,
      myReferralCode: generateReferalCode(5),
      otherReferralCode: getOtherReferralCode(),
      roles: [USER_ROLES.CUSTOMER.VALUE]
    };
...
const getOtherReferralCode = () => {
  return sessionStorage.getItem(SESSIONSTORAGE_KEYS.OTHER_REFERRAL_CODE) || undefined;
}
